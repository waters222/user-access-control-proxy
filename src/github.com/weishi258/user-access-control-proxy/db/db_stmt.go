...
var StmtInitMap = map[int]string{
	...
	STMT_INIT_USERS : "create table "+TABLE_USERS+"(id integer not null primary key, name text, password char(40), salt char(32), group_id integer not null); " +
		"create index "+TABLE_USERS+"_name_index on "+TABLE_USERS+" (name);",
	STMT_INIT_GROUPS : "create table "+TABLE_GROUPS+"(id integer not null primary key, name text, desc text);",
	STMT_INIT_SESSIONS : "create table "+TABLE_SESSIONS+"(sess text nul null primary key, user_id integer not null, last_time integer not null);",
	STMT_INIT_RULES : "create table "+ TABLE_RULES +"(id integer not null primary key, group_id integer, rule text not null,  permission tinyint not null, proxy text not null, weight integer not null, desc text);" +
		"create index "+TABLE_RULES+"_group_index on "+TABLE_RULES+" (group_id);",

}
var StmtMap = map[int]string{

	STMT_INSERT_GROUP :         "insert into "+TABLE_GROUPS+"(name, desc) values(?, ?);",
